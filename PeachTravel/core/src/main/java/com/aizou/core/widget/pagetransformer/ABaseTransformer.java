...
package com.aizou.core.widget.pagetransformer;
...
public abstract class ABaseTransformer implements PageTransformer {
	...
	protected void onPreTransform(View page, float position) {
        ...
        AnimatorProxy.wrap(page).setRotationX(0);
        AnimatorProxy.wrap(page).setRotationY(0);
        AnimatorProxy.wrap(page).setRotation(0);
        AnimatorProxy.wrap(page).setScaleX(1);
        AnimatorProxy.wrap(page).setScaleY(1);
        AnimatorProxy.wrap(page).setPivotX(0);
        AnimatorProxy.wrap(page).setPivotY(0);
        AnimatorProxy.wrap(page).setTranslationY(0);
        AnimatorProxy.wrap(page).setTranslationX(isPagingEnabled() ? 0f : -width * position);

		if (hideOffscreenPages()) {
            AnimatorProxy.wrap(page).setAlpha(position <= -1f || position >= 1f ? 0f : 1f);
		} else {
            AnimatorProxy.wrap(page).setAlpha(1f);
		}
	}
