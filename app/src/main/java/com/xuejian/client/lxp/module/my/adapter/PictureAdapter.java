 ...
 */
public class PictureAdapter extends ViewPager implements MatrixImageView.OnMovingListener {
...
//			view.getParent().bringChildToFront(view);
    ...
    /**  播放按钮点击事件 */
    public PictureAdapter(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public class LocalViewPagerAdapter extends PagerAdapter {
        private List<ImageBean> paths;//大图地址 如果为网络图片 则为大图url
        public LocalViewPagerAdapter(ArrayList<ImageBean> paths){
            this.paths=paths;
        }
