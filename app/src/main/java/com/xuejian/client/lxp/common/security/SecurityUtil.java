...
//        }
        ...
        for (int i = 0; i < request.getUrlParams().size(); i++) {
            query.append(request.getUrlParams().get(i).getName().toLowerCase())
                    .append("=")
                    .append(URLEncoder.encode(request.getUrlParams().get(i).getValue(),"UTF-8"));
        ...
        String body_str = UrlSafeBase64.encodeToString(body.getBytes("UTF-8"));
     ...
     //   StringBuilder SignatureMessage = new StringBuilder();
        ...
        if (!TextUtils.isEmpty(body)){
            signatureMessage.append(",Body="+body);
        }
